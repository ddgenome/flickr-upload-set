...
my $api_key = 'f29c841be6fbc8792edffebc4cbf0f0c';
my $not_so_secret = 'c446ffc3028f0cfa';
...
while( my $photo = shift @ARGV ) {
	...
	print "\n";
        $log_fh->print(Cwd::abs_path($photo), "\n");
}
...
if ($photoset) {
    ...
    my $req = Flickr::API::Request->new({
        ...
        args => {
            ...
            title => $photoset,
            primary_photo_id => shift(@photoids),
        },
    });
    ...
    # get photosetid
    my $photosetid;
    foreach my $n (@{$res->{tree}{children}}) {
        next unless defined $n and exists $n->{name} and $n->{name} eq 'photoset';
        $photosetid = $n->{attributes}{id};
    }
    if (!$photosetid) {
        warn("failed to determine photosetid");
        exit(11);
    }

    # add images to photoset
    foreach my $photoid (@photoids) {
        $req = Flickr::API::Request->new({
        ...
        $res = $ua->execute_request($req);
        if ($res->{success} != 1) {
            warn("failed to add photo $photoid to photoset $photoset");
            exit(12);
        }
    }
}
